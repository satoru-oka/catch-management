...
## ISSUE-004: Supabase env 未設定時のフロントエラーが分かりにくい

- **発見日**: 2026-05-18
- **発見経緯**: GitHub Issue #23 (`FS-1`)。
- **影響**:
  - `NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_ANON_KEY` 未設定のまま
    フロントを起動した時、Supabase client 生成時の低レイヤーなエラーになり、
    `.env.local` 設定漏れに気付きにくい。
- **原因**:
  - `frontend/lib/supabase.ts` が env を non-null assertion (`!`) で渡しており、
    アプリ側で設定有無を検査していなかった。
- **修正**:
  - `frontend/lib/supabase-env.ts` に env 検証 helper を追加。
  - `frontend/lib/supabase.ts` は client 作成前に必須 env を確認し、
    未設定なら
    `NEXT_PUBLIC_SUPABASE_URL / NEXT_PUBLIC_SUPABASE_ANON_KEY が設定されていません`
    を投げるように変更。
- **再発防止**:
  - `frontend/tests/supabase.test.ts` で env 未設定時の明示エラーと、
    設定済み env の読み取りをユニットテスト化。
