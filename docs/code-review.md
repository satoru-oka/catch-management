...
| D-2 | 🟡 | `SUPABASE_URL` / `SUPABASE_ANON_KEY` の型が `str \| None` のまま narrow されない。`auth.py` で `create_client(...)` に渡す時、将来 mypy/pyright を厳格化すると警告 | **DONE**: env 取得値を private 変数で検証後、`SUPABASE_URL: str` / `SUPABASE_ANON_KEY: str` として再代入 |
...
| D-5 | ⚪ | `Client` 型を `auth.py` でも別途 import している。一箇所に集約可 | **WONTFIX**: `Client` は Supabase SDK の外部型なので利用箇所で import する方が自然。`database.py` から re-export すると `auth.py` の結合が増える |
...
| A-5 | ⚪ | `HTTPBearer()` の `auto_error` を明示していない | **DONE**: `HTTPBearer(auto_error=True)` で認証必須 API の意図を明示 |
...
| L-5 | ⚪ | `routers/lures.py` 冒頭が空行 ([lures.py:1](backend/routers/lures.py#L1))。S-3 と同じ Ruff 整理残骸 | **DONE**: 先頭空行を削除 |
...
| FX-2 | 🟢 | `useEffect` の依存配列が `apiFetch` などモジュールスコープ関数を省略している箇所多数 | (protected) 配下のページほぼ全部 | **DONE**: Next.js の hooks lint を CI で `npm run lint -- --max-warnings=0` として継続実行 |
